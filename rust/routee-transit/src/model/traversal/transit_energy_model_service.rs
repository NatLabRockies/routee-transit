...
/// Dispatch service that holds a library of transit vehicle models and selects one
/// based on the `model_name` field of the incoming query.
#[derive(Clone)]
pub struct TransitEnergyModelService {
    pub vehicle_library: HashMap<String, Arc<dyn TraversalModelService>>,
}

impl TransitEnergyModelService {
    pub fn new(
        vehicle_library: HashMap<String, Arc<dyn TraversalModelService>>,
    ) -> Result<Self, TraversalModelError> {
        Ok(TransitEnergyModelService { vehicle_library })
    }
}

impl TraversalModelService for TransitEnergyModelService {
    fn build(
        &self,
