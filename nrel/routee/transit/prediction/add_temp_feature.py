...
def add_HVAC_energy(feed: Any, trips_df: pd.DataFrame) -> Any:
    ...
    df_stops = feed.stops
    gdf_stops = gpd.GeoDataFrame(df_stops, geometry = gpd.points_from_xy(df_stops.stop_lon,df_stops.stop_lat),crs = 4269)
    gdf_county = gpd.read_file("https://www2.census.gov/geo/tiger/GENZ2022/shp/cb_2022_us_county_20m.zip")
    gdf_county['county_id'] = 'G' + gdf_county['STATEFP'] + '0' + gdf_county['COUNTYFP'] + '0'
    gdf_stops = gpd.sjoin(gdf_stops, gdf_county[['geometry','county_id']],how = 'left', predicate= 'intersects')
    ...
    s3 = boto3.client('s3', config=Config(signature_version=UNSIGNED)) # Create anonymous S3 client
    save_dir = f"./TMY" # Folder for saving the downloaded TMY data
    ...
    BTMS_power_list = [4.9,3.6,2.1,0.8,0.2,0.1,0.1,1.4,1.5,2.1,5.6] # From literature
    ...
    for county_id in county_ids: 
        ...
        df_temp_group = df_temp.groupby('date').agg(avg_temp_C=('Dry Bulb Temperature [°C]', 'mean')).reset_index()
        cold_day = df_temp_group[df_temp_group.avg_temp_C == df_temp_group.avg_temp_C.min()]['date'].iloc[0]
        hot_day = df_temp_group[df_temp_group.avg_temp_C == df_temp_group.avg_temp_C.max()]['date'].iloc[0]
        # Get the coldest and hottest day data
        df_cold = df_temp[df_temp.date == cold_day].copy()
        df_cold['Dry Bulb Temperature [°C]'] = df_cold['Dry Bulb Temperature [°C]'].round(1)
        df_cold = df_cold.merge(df_temp_energy,
                                left_on='Dry Bulb Temperature [°C]',
                                right_on='Temp_C',how = 'left')
        df_cold['hour'] = np.arange(0,24)
        df_hot = df_temp[df_temp.date == hot_day].copy()
        df_hot['Dry Bulb Temperature [°C]'] = df_hot['Dry Bulb Temperature [°C]'].round(1)
        df_hot = df_hot.merge(df_temp_energy,
                            left_on='Dry Bulb Temperature [°C]',
                            right_on='Temp_C',how = 'left')
    ...
    df_stop_times = df_stop_times[df_stop_times['trip_id'].isin(trips_df['trip_id'].unique())].copy()
    ...
    df_trip_time['Winter_HVAC_Energy'] = compute_HVAC_energy(start_hours, end_hours, df_cold_hourly['Power'].values)
    df_trip_time['Summer_HVAC_Energy'] = compute_HVAC_energy(start_hours, end_hours, df_hot_hourly['Power'].values)
    # Merge back to trips_df
    trips_df = trips_df.merge(df_trip_time[['trip_id','Winter_HVAC_Energy','Summer_HVAC_Energy']],on='trip_id',how='left')
