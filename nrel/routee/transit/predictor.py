...
class GTFSEnergyPredictor:
    ...
    def add_mid_block_deadhead(self) -> Self:
        ...
        deadhead_trip_times = deadhead_stop_times.groupby("trip_id").agg(start_time=("arrival_time", "min"),
                                                                         end_time=("arrival_time", "max")).reset_index()
        
        deadhead_trip_times["trip_duration_minutes"] = ((deadhead_trip_times['end_time'] - deadhead_trip_times['start_time']).dt.total_seconds() / 60).round().astype(int)
        ...
        deadhead_trip_times["start_time"] = deadhead_trip_times["start_time"].apply(format_timedelta)
        deadhead_trip_times["end_time"] = deadhead_trip_times["end_time"].apply(format_timedelta)

        deadhead_trips = deadhead_trips.merge(
            deadhead_trip_times[["trip_id", "start_time", "end_time", "trip_duration_minutes"]],
            on="trip_id",
            how="left",
        )
        ...
        deadhead_trips['before_trip'] = deadhead_trips['trip_id'].apply(lambda x: x.split('_to_')[0])
    ...
    def add_depot_deadhead(self) -> Self:
        ...
        deadhead_trips['before_or_after_trip'] = deadhead_trips['trip_id'].apply(
            lambda x: x.split('depot_to_')[1] if 'depot_to_' in x else x.split('_to_depot')[0]
        )
        trip_counts = self.trips.set_index('trip_id')['trip_count'].to_dict()
        deadhead_trips['trip_count'] = deadhead_trips['before_or_after_trip'].map(trip_counts)
